...
if (!Array.isArray(save.planets)) save.planets = ['terra'];  // keşfedilen gezegenler
if (typeof save.planet !== 'string') save.planet = 'terra';  // seçili gezegen
...
const FB = {
  ...
  async ban(uid, opts) {
    opts = opts || {};
    const rec = {
      by: this.uid, t: Date.now(),
      code: opts.code || 'other',                    // çevrilebilir sebep kodu
      reason: (opts.reason || '').slice(0, 200),     // (eski/serbest metin desteği)
...
FB.init().then(async () => {
  ...
  await FB.detectRole();
  const banMap = await FB.fetchBans();
  const myBan = banMap.get(FB.uid);
...
const ACH = [
  ...
  { id: 'explorer',   icon: '🔭', name: 'Explorer',      desc: 'Discover 5 planets',        cond: s => (s.planets || []).length >= 5 },
  { id: 'cartograph', icon: '🪐', name: 'Cartographer',  desc: 'Discover every planet',     cond: s => (s.planets || []).length >= PLANETS.length },
...
// ==================== 🪐 GEZEGENLER (V10 — Gezegenlerin Keşfi) ====================
// Her gezegen bir palet + kendi zorluk/ödül dengesi. Koşu sırasında "need" metresi
...
// ---------- ZORLUK RİTMİ (oyuncu geri bildirimi: "arada nefes alsın") ----------
// Bir döngü: uzun yoğun bölüm → kısa "nefes" bölümü → tekrar yoğun.
// Genel zorluk yine artar (hız zamanla yükselir), ama düz bir tırmanış yerine
// dalgalı ilerler: yoruldukça rahatlama, rahatladıkça yeni meydan okuma.
const RHYTHM_LEN = 900;   // metre — bir tam döngü
let calmPhase = false;    // şu an nefes bölgesinde miyiz (bildirim için)
function calmFactor() {
  // 1 = tam yoğunluk · 0.35 = nefes bölgesi; geçişler yumuşak
  const p = (runStats.dist % RHYTHM_LEN) / RHYTHM_LEN;
  if (p < 0.60) return 1;                              // yoğun
  if (p < 0.70) return 1 - ((p - 0.60) / 0.10) * 0.65; // yumuşak iniş
  if (p < 0.90) return 0.35;                           // nefes al
  return 0.35 + ((p - 0.90) / 0.10) * 0.65;            // yumuşak yükseliş
}
...
function checkDiscovery() {
  for (const p of PLANETS) {
    if (p.need <= 0 || save.planets.includes(p.id)) continue;
    if (runStats.dist < p.need) continue;
    save.planets.push(p.id);
    save.coins += p.reward;
    runStats.coins += p.reward;
    persist();
    SFX.buy && SFX.buy(); vib([30, 60, 30]);
    try { window.bigWave(playerGroup.position, 0x7ef0ff, 26, 0.9); window.camPunch(7); } catch (e) {}
    popup('🪐 ' + T('discovered') + ': ' + p.icon + ' ' + p.name + '  +🪙' + fmt(p.reward), '#7ef0ff');
    checkAch();
    break; // aynı karede tek keşif — üst üste binmesin
  }
}
...
function resetRun() {
  ...
  distSinceSpawn = 0; distSinceScenery = 0; calmPhase = false;
  ...
  snapTheme(planetIndex()); applyThemeLerp(1);
...
$('planetsBtn').addEventListener('click', () => { SFX.ui && SFX.ui(); window.openPlanets && window.openPlanets(); });
...
function loop(now) {
  ...
  if (state === S.PLAY) {
    ...
    // ZORLUK RİTMİ: yoğun → nefes al → yoğun (her döngü bir öncekinden biraz zor)
    const calm = calmFactor();
    if (!bossActive && !eventActive) speed *= 0.82 + 0.18 * calm;
    // nefes bölgesine girip çıkışta oyuncuyu bilgilendir
    const inCalm = calm < 0.5;
    if (inCalm !== calmPhase) {
      calmPhase = inCalm;
      if (!bossActive && !eventActive && typeof popup === 'function') {
        popup(inCalm ? T('breathe') : T('speedup'), inCalm ? '#9dff70' : '#ffb37a');
      }
    }
    ...
    score += dz * planetOf().scoreMul * rocketDef.scoreMul * (1 + (droneDef && droneDef.scoreBonus ? droneDef.scoreBonus : 0)) * (fx.mult > 0 ? 2 : 1) * (fx.turbo > 0 ? 1.5 : 1);
    ...
    // 🪐 GEZEGEN KEŞFİ: bu koşuda yeni bir gezegenin mesafesine ulaşıldı mı?
    checkDiscovery();

    // tema geçişi — seçili gezegenden başlayıp sırayla ilerler
    const ti = (planetIndex() + Math.floor(runStats.dist / THEME_LEN)) % PLANETS.length;
    if (ti !== themeIdx) {
      snapTheme(ti);
      popup(PLANETS[ti].icon + ' ' + PLANETS[ti].name, '#cfd8ff');
    }
    ...
    if (eventActive === 'coinrush') {
      ...
      // ritim: nefes bölgesinde engeller seyrekleşir, arada altın sırası gelir
      const c = calmFactor();
      const gap = Math.max(26, 46 - elapsed * 0.4) * (1 + (1 - c) * 0.9);
      if (distSinceSpawn >= gap) {
        distSinceSpawn = 0;
        if (c < 0.5 && Math.random() < 0.5) spawnCoinRow(SPAWN_Z); // nefeste ödül
        else spawnWave();
      }
    }
    distSinceScenery += dz;
    ...
    for (const it of coinPool) {
      ...
      if (caught || hitsPlayer(it, 1.2)) {
        const gain = Math.round(planetOf().coinMul * rocketDef.coinMul * (dayFirstRun ? 2 : 1) * (itemDouble ? 2 : 1));
        runCoins += gain;
        runStats.coins += gain;
    ...
    rows.slice(0, 80).forEach(r => {
      ...
      if (!prot) {
        ...
        bb.onclick = async () => {
          ...
          // Sebep KOD olarak seçilir → banlanan kişiye KENDİ dilinde gösterilir
          const codes = ['cheat', 'spam', 'abuse', 'name', 'other'];
          const pick = prompt('Ban sebebi:\n1) Hile / skor manipülasyonu\n2) Spam / reklam\n3) Hakaret / kötü davranış\n4) Uygunsuz kullanıcı adı\n5) Diğer', '1');
          if (pick === null) { bb.textContent = '🔨'; return; }
          const code = codes[(parseInt(pick, 10) || 1) - 1] || 'other';
          bb.textContent = '…';
          const ok = await FB.ban(r.id, { permanent: perm, days: 7, code: code });
...
// ==================== 🚫 BAN EKRANI (banlı kullanıcı oyuna giremez) ====================
// Siyah tam ekran + kırmızı kalkan + "BANLANDINIZ" + sebep (kullanıcının dilinde)
// + kalan süre. Hiçbir şeye basılamaz; kapatma yolu yoktur.
window.showBanScreen = function (rec) {
  if (document.getElementById('banScreen')) return;
  rec = rec || {};
  const wrap = document.createElement('div');
  wrap.id = 'banScreen';
  wrap.style.cssText = 'position:fixed;inset:0;z-index:99999;background:#000;color:#fff;' +
    'display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;' +
    'padding:24px;font-family:"Segoe UI",Arial,sans-serif;user-select:none';

  // kırmızı kalkan (SVG — her cihazda aynı görünür, emoji fontuna bağlı değil)
  const shield = document.createElement('div');
  shield.style.cssText = 'margin-bottom:14px;filter:drop-shadow(0 0 22px rgba(225,29,56,.85))';
  shield.innerHTML =
    '<svg width="104" height="118" viewBox="0 0 104 118" fill="none" xmlns="http://www.w3.org/2000/svg">' +
    '<path d="M52 4 L96 20 V58 C96 86 76 104 52 114 C28 104 8 86 8 58 V20 Z" fill="#e11d38" stroke="#ff5566" stroke-width="4" stroke-linejoin="round"/>' +
    '<path d="M52 16 L86 28 V58 C86 80 70 94 52 102 C34 94 18 80 18 58 V28 Z" fill="#8f0d20" opacity=".55"/>' +
    '<path d="M36 44 L68 76 M68 44 L36 76" stroke="#fff" stroke-width="9" stroke-linecap="round"/>' +
    '</svg>';
  wrap.appendChild(shield);

  const title = document.createElement('div');
  title.textContent = T('banned_title');
  title.style.cssText = 'font:800 30px/1.2 "Segoe UI";color:#ff2b45;letter-spacing:1px;' +
    'text-shadow:0 0 18px rgba(225,29,56,.7);margin-bottom:6px';
  wrap.appendChild(title);

  const note = document.createElement('div');
  note.textContent = T('banned_note');
  note.style.cssText = 'font-size:13px;color:#8a8a94;margin-bottom:22px';
  wrap.appendChild(note);

  // sebep (kod varsa kullanıcının diline çevrilir; yoksa serbest metin)
  const reasonTxt = rec.code ? T('br_' + rec.code) : (rec.reason || T('br_other'));
  const rBox = document.createElement('div');
  rBox.style.cssText = 'background:#140508;border:1px solid #4a1020;border-radius:12px;' +
    'padding:14px 18px;margin-bottom:12px;max-width:340px;width:100%';
  const rLabel = document.createElement('div');
  rLabel.textContent = T('banned_reason');
  rLabel.style.cssText = 'font-size:11px;color:#8a8a94;letter-spacing:1px;margin-bottom:4px';
  const rVal = document.createElement('div');
  rVal.textContent = reasonTxt;               // textContent → XSS yok
  rVal.style.cssText = 'font:700 16px "Segoe UI";color:#ffb3bd';
  rBox.appendChild(rLabel); rBox.appendChild(rVal);
  wrap.appendChild(rBox);

  // süre kutusu
  const tBox = document.createElement('div');
  tBox.style.cssText = 'background:#140508;border:1px solid #4a1020;border-radius:12px;' +
    'padding:14px 18px;max-width:340px;width:100%';
  const tLabel = document.createElement('div');
  tLabel.textContent = T('banned_left');
  tLabel.style.cssText = 'font-size:11px;color:#8a8a94;letter-spacing:1px;margin-bottom:4px';
  const tVal = document.createElement('div');
  tVal.style.cssText = 'font:700 18px "Segoe UI";color:#ff2b45';
  tBox.appendChild(tLabel); tBox.appendChild(tVal);
  wrap.appendChild(tBox);

  function fmtLeft() {
    if (rec.permanent === true || !rec.until) { tVal.textContent = T('banned_perm'); return; }
    const ms = rec.until - Date.now();
    if (ms <= 0) { location.reload(); return; }   // süre doldu → oyun geri açılır
    const d = Math.floor(ms / 864e5), h = Math.floor((ms % 864e5) / 36e5), m = Math.floor((ms % 36e5) / 6e4);
    tVal.textContent = d > 0 ? (d + ' ' + T('d_day') + ' ' + h + ' ' + T('d_hour'))
      : h > 0 ? (h + ' ' + T('d_hour') + ' ' + m + ' ' + T('d_min'))
      : (m + ' ' + T('d_min'));
  }
  fmtLeft();
  setInterval(fmtLeft, 30000);

  document.body.appendChild(wrap);
  // sesi kes ve oyunu durdur
...
// ==================== 🪐 GEZEGEN GALERİSİ (V10) ====================
(function () {
  const st = document.createElement('style');
  st.textContent =
    '#plWrap{position:fixed;inset:0;z-index:120;display:none;align-items:center;justify-content:center;background:rgba(3,1,12,.92);backdrop-filter:blur(4px)}' +
    '#plWrap.on{display:flex}' +
    '#plCard{width:min(94vw,460px);max-height:88vh;overflow-y:auto;background:linear-gradient(180deg,#140a28,#0a0518);' +
    'border:2px solid #7ef0ff55;border-radius:20px;padding:16px;color:#fff;box-shadow:0 12px 50px rgba(0,0,0,.7)}' +
    '#plCard h2{text-align:center;font:800 22px/1.2 "Segoe UI";margin-bottom:2px;' +
    'background:linear-gradient(90deg,#7ef0ff,#b388ff);-webkit-background-clip:text;background-clip:text;color:transparent}' +
    '#plSub{text-align:center;font-size:12px;color:#8fa;margin-bottom:12px}' +
    '.plItem{display:flex;gap:12px;align-items:center;background:#1a1030;border:1px solid #2c1f4a;border-radius:14px;' +
    'padding:11px 12px;margin:8px 0;transition:transform .08s}' +
    '.plItem.sel{border-color:#7ef0ff;box-shadow:0 0 18px #7ef0ff33}' +
    '.plItem.locked{opacity:.5}' +
    '.plItem:active{transform:scale(.985)}' +
    '.plIcon{font-size:34px;width:46px;text-align:center;flex:none}' +
    '.plInfo{flex:1;min-width:0}' +
    '.plName{font:800 15px "Segoe UI";letter-spacing:.5px}' +
    '.plDesc{font-size:11.5px;color:#a9a3bb;margin-top:2px}' +
    '.plMul{font-size:11px;color:#ffd54d;margin-top:3px}' +
    '.plBtn{flex:none;border:none;border-radius:10px;padding:9px 13px;font:800 12px "Segoe UI";cursor:pointer;color:#fff}' +
    '#plClose{width:100%;margin-top:12px;padding:12px;border:2px solid #445;border-radius:12px;background:transparent;color:#ccd;font:800 14px "Segoe UI";cursor:pointer}';
  document.head.appendChild(st);

  const wrap = document.createElement('div');
  wrap.id = 'plWrap';
  wrap.innerHTML = '<div id="plCard"><h2>🪐 ' + escHTML(T('planets')) + '</h2><div id="plSub"></div><div id="plList"></div><button id="plClose"></button></div>';
  document.body.appendChild(wrap);
  const list = wrap.querySelector('#plList'), sub = wrap.querySelector('#plSub'), closeB = wrap.querySelector('#plClose');
  closeB.onclick = () => { SFX.ui && SFX.ui(); wrap.classList.remove('on'); };
  wrap.onclick = e => { if (e.target === wrap) wrap.classList.remove('on'); };

  function render() {
    closeB.textContent = T('mainmenu');
    sub.textContent = T('discovered') + ': ' + save.planets.length + '/' + PLANETS.length +
      '  ·  ' + T('best') + ': ' + fmt(Math.floor(save.best || 0)) + ' m';
    list.innerHTML = '';
    for (const p of PLANETS) {
      const found = save.planets.includes(p.id);
      const sel = save.planet === p.id;
      const row = document.createElement('div');
      row.className = 'plItem' + (sel ? ' sel' : '') + (found ? '' : ' locked');

      const ic = document.createElement('div');
      ic.className = 'plIcon';
      ic.textContent = found ? p.icon : '❔';
      row.appendChild(ic);

      const info = document.createElement('div');
      info.className = 'plInfo';
      const nm = document.createElement('div');
      nm.className = 'plName';
      nm.textContent = found ? p.name : '???';
      const ds = document.createElement('div');
      ds.className = 'plDesc';
      ds.textContent = found ? p.desc : (T('reach') + ' ' + fmt(p.need) + ' m');
      ...
      btn.className = 'plBtn';
      if (!found) { btn.textContent = '🔒'; btn.style.background = '#332b47'; btn.disabled = true; }
      else if (sel) { btn.textContent = '✓'; btn.style.background = 'linear-gradient(135deg,#2ea86a,#166b3a)'; }
      else {
        btn.textContent = T('select');
        btn.style.background = 'linear-gradient(135deg,#2f6fe0,#1a3a8f)';
        btn.onclick = () => { save.planet = p.id; persist(); SFX.ui && SFX.ui(); vib(20); render(); if (typeof showMenu === 'function') showMenu(); };
      }
      row.appendChild(btn);
      list.appendChild(row);
    }
  }
  window.openPlanets = function () { render(); wrap.classList.add('on'); };
})();
